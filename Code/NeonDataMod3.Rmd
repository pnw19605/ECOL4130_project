...
chlorophyll_mosquito_2018 %>%
  drop_na%>%
  rename(`Mosquito Abundance`=mosquito_abundance)%>%
  rename(`Chlorophyll A (mg/L)`=chlorophyll_a)%>%
  ggplot(aes(x=`Chlorophyll A (mg/L)`, y= `Mosquito Abundance`))+
  geom_point()+
...
chlorophyll_mosquito_all %>%
  ...
  ggtitle("Effect of Chlorophyll A levels on Mosquito Abundance")+
  theme(plot.title=element_text(hjust=0.5))

```
Figure 1: Scatter plot showing the combined data of chlorophyll-A concentration and mosquito abundance for 2018 and 2019. There is a slightly negative correlation between mosquito abundance and chlorophyll-a concentration.
...
```
Test interpretation:
The higher p-value for the "all data" set suggests that there is no significance between the variables. This test does not align with the predictions of our hypothesis.
...
chlorophyll_mosquito_all %>%
  ...
  rename(Month=month)%>%
  rename(Year=year)%>%
  ggplot(aes(x=Month, y=`Chlorophyll A (mg/L)`, color=Year, group=Year))+
  geom_point()+
  geom_line()+
  theme_classic()+
  ggtitle("Chlorophyll A Concentration by Month")+
  theme(plot.title=element_text(hjust=0.5))+
...
```
Figure 2: Point plot representing chlorophyll-a concentration over time (in months) for both the years 2018 (orange) and 2019 (blue). Chlorophyll-a concentrations in 2019 appear to be lower in all months than 2018.
...
chlorophyll_mosquito_all %>%
  ...
  ggplot(aes(x=Month, y=`Mosquito Abundance`, color=Year, group=Year))+
  geom_point()+
  geom_line()+
  theme_classic()+
  ggtitle("Mosquito Abundance Concentration by Month")+
  theme(plot.title=element_text(hjust=0.5))+
  theme(axis.text.x=element_text(vjust=0.5,angle=45))+
  theme(legend.title=element_text(hjust=0.5))
...
Figure 3: Point plot representing the mosquito abundance over time (in months) for both the years 2018 (orange) and 2019 (blue). No apparent pattern can be seen.